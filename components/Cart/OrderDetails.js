...
const OrderDetails = () => {
    ...
    useEffect(() => {
        let subt = allCartItems.length ? allCartItems.map(itm => itm.base_price * itm.quantity).reduce((prev, curr) => prev + curr) : 0
        setSubTotal(subt)
    }, [allCartItems])
                ...
                {!allCartItems.length
                    ...
                    <div className="p-4">
                        ...
                        <div className="flex justify-between pt-4 border-b">
                            ...
                            <div className="lg:px-0 lg:py-0 m-2 lg:text-lg font-medium text-center text-gray-900">
                                $ {(newSubTotal + ((tax * newSubTotal) / 100)).toFixed(2)}
                            </div>
                        </div>
                        <div className=''>
                            <Button clickFunc={() => router.push('/checkout')} classAdd="flex items-center gap-2 justify-center w-full px-10 py-3 mt-6 font-medium text-white uppercase  shadow ">
