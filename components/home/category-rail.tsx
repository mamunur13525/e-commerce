    ...
    <section className="container mx-auto px-6 md:px-12 py-12">
      <div className="flex items-center justify-between gap-4">
        <div className="flex lg:flex-row flex-col w-full gap-4">
          <div
            className={`flex-1 grid gap-4 ${categories.length === 1
              ? "grid-cols-1"
              : categories.length === 2
          ...
          >
            {categories?.slice(0, 4).map((cat: Category) => (
              <Link
                ...
                className="w-full bg-white flex items-center gap-3 px-4 pt-7 pb-14 rounded-xl transition-transform hover:scale-105 group relative shadow-lg shadow-zinc-200"
              >
