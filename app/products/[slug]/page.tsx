...
import { StarIcon, PlusSignIcon, Remove01Icon, ArrowLeft01Icon } from "hugeicons-react";
...
export default function ProductPage() {
        ...
        return (
            <main className="min-h-screen bg-white flex items-center justify-center">
                <div className="text-center max-w-md mx-auto px-4">
                    ...
                    <p className="text-gray-600 mb-6">{error || "Unable to load product details"}</p>
                    <Link
                        href="/"
    ...
    const storeName = typeof product.store === 'string' ? product.store : product.store?.name || 'Unknown Store';
        ...
        <main className="min-h-screen bg-white">
            <div className="container mx-auto px-4 md:px-8 py-8 space-y-12">
                ...
                <nav className="flex items-center gap-2 text-sm text-gray-600">
                    <Link href="/" className="hover:text-[#003d29]">Home</Link>
                    <span>/</span>
                    <Link href="/shop" className="hover:text-[#003d29]">Shop</Link>
                    <span>/</span>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16">
                    ...
                    <div className="space-y-4">
                        ...
                        >
                            ...
                            <Image
                                key={selectedImage}
                                src={allImages[selectedImage]?.url || allImages[selectedImage]?.display_url || product.image.url}
                                alt={product.name}
                                fill
                    ...
                    <div className="space-y-5">
                        ...
                        <div className="flex items-center gap-1 mb">
                            <div className="flex items-center">
                                {[...Array(5)].map((_, i) => (
                                    <StarIcon
                                        key={i}
                                        className={`size-4 ${i < Math.floor(product.rating || 0) ? 'text-yellow-400 fill-yellow-400' : 'text-gray-300 fill-gray-300'}`}
                                    />
                                ))}
                            </div>
                        ...
                        {product.discount && product.discount > 0 && (
                            <div className="flex items-center gap-3">
                                ...
                                <span className="text-sm font-medium text-red-600">
                                    Save {getCurrencySymbol(product.currency)}{(product.price - discountedPrice).toFixed(2)}
                                </span>
                            </div>
                        )}
                        ...
                        <div className="flex gap-3 pt-4">
                            <Button
                                variant="outline"
                                onClick={() => {
                                    const imageElement = document.querySelector(`img[alt="${product.name}"]`) as HTMLImageElement;
                                    if (imageElement) {
                                        const rect = imageElement.getBoundingClientRect();
                                        const imageSrc = product.image.url || product.image.display_url || '';
                        ...
                        <div className="flex items-center gap-3 pt-2">
                            ...
                            <Button
                                ...
                                onClick={() => {
                                    toast.success('Success Sonner');
                                    toast.error('Error Sonner');
                                    toast.info('Info Sonner');
                        ...
                        {(product.quantity || 0) > 0 && (
                            <div className="flex items-center gap-2 text-sm pt-2">
                                ...
                                <span className="text-red-600 font-medium">100 sold in last 35 hour</span>
                            </div>
                        )}
                        ...
                        <div className="space-y-2 pt-4 border-t">
                            <div className="text-sm">
                                ...
                                <span className="text-gray-600 ml-2">{product._id.slice(-8).toUpperCase()}</span>
                            </div>
                            <div className="text-sm">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-6">
                            <div className="flex items-start gap-3">
                                ...
                                <div>
                                    <h3 className="font-semibold text-gray-900">Free Delivery <span className="font-normal text-gray-500">Apply To</span></h3>
                                    <p className="text-sm text-gray-500 mt-1">All Order Over $100</p>
                                </div>
                            </div>
                            <div className="flex items-start gap-3">
                                <div className="text-orange-500 mt-1">
                                ...
                                <div>
                                    <h3 className="font-semibold text-gray-900">Great Daily Deal <span className="font-normal text-gray-500">We</span></h3>
                                    <p className="text-sm text-gray-500 mt-1">Providing Organic Products</p>
                                </div>
                            </div>
                        </div>
